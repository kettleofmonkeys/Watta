function [heights, diff2_line, probabilities, isLake, isFound] = subfn_pull_subset_for_estimate_il_sub( midPoint_index, photonCount, bandwidth, s_heights)

    % this function is the meat of the operation
    % it selects the number of photons within a width and then fits
    % it to a kernel density estimate with a bandiwidth (think bin width)
    ...
    [startOrig, endOrig, heights_list_sub] = subfn_select_subset_photons_redo(midPoint_index, photonCount, s_heights);  %%% select a number of photons (photonCount) left and right of the midpoint photon
    ...
    %second derivative (diff2_line) and probabilities, which will indicate
